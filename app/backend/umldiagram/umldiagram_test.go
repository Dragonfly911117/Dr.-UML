...
func TestBaseCommandTimestamps(t *testing.T) {
	...
	// Test GetBefore
	before := cmd.GetBefore()
	assert.Equal(t, cmd.before, before)

	// Test GetAfter
	after := cmd.GetAfter()
	assert.Equal(t, cmd.after, after)
}

func TestSelectAllCommand(t *testing.T) {
	diagram, err := CreateEmptyUMLDiagram("SelectAllTest.uml", ClassDiagram)
	assert.NoError(t, err)
	assert.NotNil(t, diagram)

	// Add two gadgets
	err = diagram.AddGadget(component.Class, utils.Point{X: 10, Y: 20}, 0, drawdata.DefaultGadgetColor, "Gadget1")
	assert.NoError(t, err)
	err = diagram.AddGadget(component.Class, utils.Point{X: 100, Y: 200}, 0, drawdata.DefaultGadgetColor, "Gadget2")
	assert.NoError(t, err)
	...
	// Create a map of components to select
	compsToSelect := make(map[component.Component]bool)
	for _, comp := range components {
		compsToSelect[comp] = true
	}

	// Create a selectAllCommand
	cmd := &selectAllCommand{
		baseCommand: baseCommand{
			diagram: diagram,
			before:  time.Now(),
		},
		components: compsToSelect,
		newValue:   true,
	}
	...
	assert.Equal(t, len(compsToSelect), len(diagram.componentsSelected))
	for comp := range compsToSelect {
		_, selected := diagram.componentsSelected[comp]
	...
	// Verify all components are unselected
	assert.Equal(t, 0, len(diagram.componentsSelected))
}

func TestHasUnsavedChanges(t *testing.T) {
	diagram, err := CreateEmptyUMLDiagram("UnsavedChangesTest.uml", ClassDiagram)
	assert.NoError(t, err)
	assert.NotNil(t, diagram)
	...
	// Add a gadget to create an unsaved change
	err = diagram.AddGadget(component.Class, utils.Point{X: 10, Y: 20}, 0, drawdata.DefaultGadgetColor, "Gadget1")
	assert.NoError(t, err)
	...
	// Save the diagram
	_, err = diagram.SaveToFile("UnsavedChangesTest.uml")
	assert.NoError(t, err)
...
func TestCollectGadgetsAndAssociations(t *testing.T) {
	diagram, err := CreateEmptyUMLDiagram("CollectTest.uml", ClassDiagram)
	assert.NoError(t, err)
	assert.NotNil(t, diagram)

	// Add two gadgets
	gadPoint1 := utils.Point{X: 10, Y: 20}
	gadPoint2 := utils.Point{X: 100, Y: 200}
	err = diagram.AddGadget(component.Class, gadPoint1, 0, drawdata.DefaultGadgetColor, "Gadget1")
	assert.NoError(t, err)
	err = diagram.AddGadget(component.Class, gadPoint2, 1, drawdata.DefaultGadgetColor, "Gadget2")
	assert.NoError(t, err)
	...
	// Create a SavedDiagram to collect into
	savedDiagram := &utils.SavedDiagram{
		Filetype:     int(ClassDiagram),
		LastEdit:     time.Now().Format(time.RFC3339),
		Gadgets:      []utils.SavedGad{},
		Associations: []utils.SavedAss{},
	}

	// Test collectGadgets
	gadgetMap, err := diagram.collectGadgets(savedDiagram)
	assert.NoError(t, err)
	assert.Equal(t, 2, len(gadgetMap))
	assert.Equal(t, 2, len(savedDiagram.Gadgets))

	// Verify gadget properties were collected correctly
	for _, gadget := range savedDiagram.Gadgets {
		assert.Contains(t, []int{0, 1}, gadget.Layer)
		assert.Contains(t, []int{int(component.Class)}, gadget.GadgetType)
		assert.Contains(t, []string{drawdata.DefaultGadgetColor}, gadget.Color)
	}

	// Test collectAssociations
	err = diagram.collectAssociations(gadgetMap, savedDiagram)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(savedDiagram.Associations))

	// Verify association properties were collected correctly
	assert.Equal(t, int(component.Composition), savedDiagram.Associations[0].AssType)
	assert.Equal(t, 2, len(savedDiagram.Associations[0].Parents))
}
...
func TestCollectGadgetsAndAssociations(t *testing.T) {
	...
	// Create a SavedDiagram to collect into
	savedDiagram := &utils.SavedDiagram{
		Name:        "CollectTest.uml",
		DiagramType: int(ClassDiagram),
		Gadgets:     []utils.SavedGad{},
		Associations: []utils.SavedAss{},
	}
